...
For a swipe-to-home on a Face ID device, you can use `pnpm --filter @repo/mobile exec serve-sim button swipe_home` — it issues the correct edge-3 touch sequence for you.
...
```sh
pnpm --filter @repo/mobile exec serve-sim tap 0.5 0.5
```
...
```sh
pnpm --filter @repo/mobile exec serve-sim gesture '{"type":"begin","x":0.5,"y":0.2}'
pnpm --filter @repo/mobile exec serve-sim gesture '{"type":"move","x":0.5,"y":0.5}'
pnpm --filter @repo/mobile exec serve-sim gesture '{"type":"move","x":0.5,"y":0.8}'
pnpm --filter @repo/mobile exec serve-sim gesture '{"type":"end","x":0.5,"y":0.8}'
```
...
```sh
pnpm --filter @repo/mobile exec serve-sim gesture '{"type":"begin","x1":0.4,"y1":0.5,"x2":0.6,"y2":0.5}'
pnpm --filter @repo/mobile exec serve-sim gesture '{"type":"move","x1":0.25,"y1":0.5,"x2":0.75,"y2":0.5}'
pnpm --filter @repo/mobile exec serve-sim gesture '{"type":"end","x1":0.25,"y1":0.5,"x2":0.75,"y2":0.5}'
```

### Pull down Notification Center (top edge)

```sh
pnpm --filter @repo/mobile exec serve-sim gesture '{"type":"begin","x":0.5,"y":0.02,"edge":2}'
pnpm --filter @repo/mobile exec serve-sim gesture '{"type":"move","x":0.5,"y":0.4,"edge":2}'
pnpm --filter @repo/mobile exec serve-sim gesture '{"type":"end","x":0.5,"y":0.4,"edge":2}'
```

### Swipe back from left edge

```sh
pnpm --filter @repo/mobile exec serve-sim gesture '{"type":"begin","x":0.01,"y":0.5,"edge":1}'
pnpm --filter @repo/mobile exec serve-sim gesture '{"type":"move","x":0.5,"y":0.5,"edge":1}'
pnpm --filter @repo/mobile exec serve-sim gesture '{"type":"end","x":0.5,"y":0.5,"edge":1}'
```
