...
These are the **only** values accepted by `pnpm --filter @repo/mobile exec serve-sim button <name>`. Anything else prints `[hid] Unknown button: <name>` and does nothing.
...
These are the **only** values accepted by `pnpm --filter @repo/mobile exec serve-sim rotate <orientation>`:
